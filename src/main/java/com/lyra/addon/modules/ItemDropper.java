    ...
    private void onTick(TickEvent.Post event) {
        ...
        for (int i = 9; i < 9 + speed.get(); i++) {
            ...
            for (RegistryKey<Enchantment> enchantKey : enchants.get()) {
                Optional<RegistryEntry.Reference<Enchantment>> enchantEntry = enchantmentRegistry.getEntry(enchantKey.getValue());
                enchantEntry.ifPresent(enchantmentReference -> itemStack.addEnchantment(enchantmentReference, enchantmentLevel.get()));
            }
