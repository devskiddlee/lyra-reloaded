...
public class RenameCommand extends Command {
    ...
    public void build(LiteralArgumentBuilder<CommandSource> builder) {
        builder.executes(context -> {
            info("§e.rename <name>");
            info("§7To color text, use . (e.g. '.cHello' or '.lNice').");
            info("Every '_' gets replaced with a space!");
    ...
    private void setCustomName(String customName) {
        ...
        ItemStack heldItemStack = mc.player.getInventory().getSelectedStack().copy();

        if (!heldItemStack.isEmpty()) {
            ItemStack itemStack = heldItemStack.copy();
            itemStack.remove(DataComponentTypes.CUSTOM_DATA);
            itemStack.set(DataComponentTypes.CUSTOM_NAME, Text.literal(customName.replaceAll("\\.", "§").replaceAll("_", " ")));
            SetItem.setMainHand(itemStack);
            mc.player.dropSelectedItem(true);
            info("§aItemStack renamed.");
        }
